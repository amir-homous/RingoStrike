...
<script setup>
import { computed } from 'vue';import BaseCard from '@/components/ui/BaseCard.vue';
const props=defineProps({days:{type:Array,default:()=>[]}});
const by=computed(()=>Object.fromEntries(props.days.map(d=>[d.date,d.count])));
const cells=computed(()=>{const out=[];const now=new Date();for(let i=139;i>=0;i--){const d=new Date(now);d.setDate(now.getDate()-i);const key=d.toISOString().slice(0,10);const c=by.value[key]||0;const ii=c>=4?4:c>=3?3:c>=2?2:c>=1?1:0;out.push({date:key,count:c,i:ii});}return out;});
</script>
